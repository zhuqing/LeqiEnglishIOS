...
class ContentInfoDataCache: DataCache<[Segment]> {
    ...
    static let CACHE_MANAGER_ID = "ContentInfoViewModel"
...
//        contentCache.getFromService() {(c) in
...
//        }
    ...
    func update(segment:Segment){
        guard  let datas = self.cacheData else{
            return
        }
      let index =  CollectionUtils.indexof(datas: datas, d: segment)
        if(index == -1){
            return
        }
        self.cacheData?[index] = segment
    }
